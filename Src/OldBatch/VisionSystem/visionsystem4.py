...
class MMI:
   ...
   def prepareMmiFolder(self, consolePath, testPath):
      ...
      copyLocalBinaries, sourceRoot = self.useLocalMmiSources()
   ...
   def getCameraSizeForIS(self, inspectionStation):
      camera = self.getPhysicalCameraForIS(inspectionStation)
      if camera is None:
         return None, None

      return self.getCameraSize(camera)
   ...
   def getCalibrationFileForIS(self, inspectionStation):
      ...
      calFile = "XY_CAL{0:02d}.XML".format(logicalCam)
      return os.path.join(self.mmiPath, "Cal", calFile)

   def getPixelResolutionForIS(self, inspectionStation):
      cal = self.getCalibrationFileForIS(inspectionStation)
      if not os.path.exists(cal):
         return None

      print "Loading calibration for", inspectionStation, "from", cal
   ...
   def createImageForIS(self, inspectionStation, componentImage, testPath, sequenceNr, lighting):
      ...
      cam = self.getPhysicalCameraForIS(inspectionStation)
      fov = self.getCameraSizeForIS(inspectionStation)
      res = self.getPixelResolutionForIS(inspectionStation)
   ...
   def startSol4Win(self):
      return # disable pre booting for now
      print "Starting Sol4Win..."
      boards = self.readCnfContent().findall("./MVSInfos/MVSInfo")
      for board in boards:
         slotName = "slot" + board.get('ID')
         base = os.path.join(self.mvsPath, slotName)
         subprocess.Popen(os.path.join(base, slotName + ".bat"), cwd=base, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
   ...
   def copyMmiBinaries(self, sourceRoot):
      ...
      def copyFilesByType(source, target, fileTypes):
         assert type(fileTypes) == list
         ostools.copyFilesByRegex(source, target, r"^.*\.(" + "|".join(fileTypes) + ")$", overwrite=True)
      ...
      def copyReelStatReporter(ccrFolder, binAndConfig):
         ...
         if os.path.exists(statsChippacBuildFolder):
            ...
            copyFilesByType(statsChippacBuildFolder, self.mmiPath, [FileType.EXE, FileType.DLL, FileType.PDB])
            copyFilesByType(statsChippacXslFolder, mmiXslFolder, [FileType.XSL])
         else:
      ...
      def copyIntelTrayReporter(ccrFolder, binAndConfig):
         ...
         if os.path.exists(ITTBuildFolder):
            ...
            copyFilesByType(ITTBuildFolder, self.mmiPath, [FileType.EXE, FileType.DLL, FileType.PDB])
         else:
      ...
      def copyDatamatrixDlls(sourceRoot):
         ...
         copyFilesByType(source, destination , [FileType.DLL])
      ...
      # MMI
      copyFilesByType(os.path.join(sourceRoot, defaultLocation, configuration),
                      self.mmiPath,
                      [FileType.EXE, FileType.DLL, FileType.PDB])

      copyFilesByType(os.path.join(sourceRoot, defaultLocation,  configuration, "convert"),
      ...
      copyFilesByType(os.path.join(sourceRoot, "installShield_setup"), self.mmiPath, [FileType.EXE]) # Job list
      copyFilesByType(os.path.join(sourceRoot, "license"), self.mmiPath, [FileType.DLL])
      copyFilesByType(os.path.join(sourceRoot, "ocrtools"), self.mmiPath, [FileType.DLL])

      # CDA reporters
      copyFilesByType(os.path.join(sourceRoot, defaultLocation, configuration, "reporters"),
                      self.mmiPath,
   ...
   def changePoolSettings(self, consolePath, testPath):
      self.changePoolSetting([["General", "CompleteRescan", "1"],
                              ...
                              ["MVS6000Boot", "BootOnStartup", "1"],  # disable pre booting for now.
